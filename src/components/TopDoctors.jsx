...
const TopDoctors = () => {
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900  md:mx-10'>
            ...
            {doctors.slice(0,10).map((item,index)=>(
                <div onClick={()=>navigate(`/appointment/${item._id}`)}  key={index} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500'>
                    ...
                    <div className='p-4'>
                      ...
                      <p className='text-gray-600 text- ' >{item.speciality}</p>
                    </div>
                </div>
        ...
        <button onClick={()=>{ navigate('/doctors'); scrollTo(0,0)}} className='text-gray-600 bg-blue-100 rounded-full px-12 py-3 mt-10'>more</button>
